...
def _plot(fig, height=360, key=None):
    """Render chart via components.html with copy button in modebar."""
    fig.update_layout(
...
def _bar_or_pie(values, names, label, key):
    ...
    if chart_type == "עוגה":
        # Use go.Pie directly for reliable JSON serialization in copy button
        ...
        # Use go.Bar with plain lists — avoids Hebrew column name issues in JSON
        ...
        fig.update_layout(
            ...
            yaxis=dict(range=[0, max(vals_sorted) * 1.2 if vals_sorted else 1]),
        )
...
def _wordcloud(series: pd.Series):
    ...
    if not words:
        return None
    # Count frequencies ourselves → use generate_from_frequencies to avoid bigrams
    freq = Counter(words)
    # Apply bidi per word so Pillow renders Hebrew correctly (no phrase reordering)
...
k2.metric("חשיפה כוללת",  f"{int(_reach_num.sum()):,}" if _reach_num.sum()>0 else "—")
k3.metric("ערך כולל (₪)", f"{int(_value_num.sum()):,}" if _value_num.sum()>0 else "—")
...
with tab_charts:
    if df.empty:
        ...
        fig_tl = go.Figure(go.Bar(
            x=list(tl["_date"]), y=list(tl["ספירה"]),
            marker_color=ST_PRIMARY, name="פרסומים",
        ))
        fig_tl.update_layout(xaxis_title="תאריך", yaxis_title="פרסומים")
        ...
        _plot(fig_tl, height=340, key="timeline")
        ...
        with c3:
            ...
            if chart_type_sent=="עוגה":
                ...
                sc_vals   = list(sc.values)
                sc_colors = [SENT_COLORS.get(s, ST_PRIMARY) for s in sc_labels]
                fig = go.Figure(go.Bar(x=sc_labels, y=sc_vals, marker_color=sc_colors))
        ...
        n_sources = 35 - _raw_src  # invert for RTL: drag right = more
        ...
        if chart_type_src=="עוגה":
            ...
            src_names = list(src["גוף"])
            src_vals  = list(src["ספירה"])
        ...
        n_words = 35 - _raw_words  # invert for RTL
        w1,w2 = st.columns(2)
        with w1:
            st.caption("בכותרות")
            ...
            if not wdf_t.empty:
                ...
                fig=go.Figure(go.Bar(x=list(wdf_t["ספירה"]),y=list(wdf_t["מילה"]),
                                     orientation="h",marker_color=palette_wt))
        ...
        with w2:
            ...
            if not wdf_c.empty:
                ...
                fig=go.Figure(go.Bar(x=list(wdf_c["ספירה"]),y=list(wdf_c["מילה"]),
                                     orientation="h",marker_color=palette_wc))
...
with tab_search:
    ...
    tbl = (
        ...
        "</style>"
        '<div style="max-height:500px;overflow-y:auto;border:1px solid #d8c4e8;border-radius:6px;direction:ltr;">'
